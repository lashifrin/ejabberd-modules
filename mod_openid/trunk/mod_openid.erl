	...
	case lists:keysearch("openid.return_to", 1, Query) of
		{value, {_, ReturnTo} } ->
			case catch verify_id(Request,Profile) of 
				...
				{return , Value } -> Value;
				_ -> redirect_reply([{"openid_mode","error"}, {"openid_error","InternalError"}], ReturnTo)
			end;
...
checkid2(#request{ q = Query} = _Request, Profile,ReturnTo) ->
	case lists:keysearch("openid.identity", 1, Query) of
		{value, {_, Ident } } when Ident ==  Profile#profile.identity -> 
			{AssocHandle,Secret} = case lists:keysearch("openid.assoc_handle", 1, Query) of
			...
			TrustRoot = case lists:keysearch("openid.trust_root", 1, Query) of
				{value, {_, Vs} } -> Vs;
				false -> ReturnTo
				end,
			...
			redirect_reply( 
			   lists:map( fun({K,V}) -> {"openid_" ++ K , V}   end  , Params)
				++ [{"openid_mode","id_res"},{"openid_signed", Signed },{"openid_sig", Sig }]
					, ReturnTo) ;
		_ -> redirect_reply([{"openid_mode","error"},{"openid_error","WrongIdentity"}],ReturnTo)
	end.

check_authentication(Query,Profile) ->
			...
			case lists:keysearch("openid.sig", 1, Query) of
				{value, {_, Sig} } -> 
					case lists:keysearch("openid.signed", 1, Query) of
						{value, {_, Signed} } ->  
							direct_reply([{"openid.mode","id_res"},{"is_valid",check_authentication2(AssocHandle, Sig, Signed, Query)}]);
...
error_400(Message) ->
 	{400, [], ejabberd_web:make_xhtml([{xmlelement,"h1",[],[{xmlcdata, "400 Bad Request"}]},{xmlelement,"p",[],[{xmlcdata, Message}]}])}.

%Ask the user agent to go to the ReturnTo URL with the specified Params in the query
...
% Given a list of {Key,Value}, construct the query string, starting with Delim (either '?' or '&')
...
%Given a list of {Key,Value} , return the string which should appears in the dirrect reply.
...
make_signature(Param , Secret) ->
	{field_list(Param) , jlib:encode_base64(binary_to_list(crypto:sha_mac(Secret, build_reply(Param))))}.

% Given a list of parameters, return a string containing the list of key separated by comas.
field_list([]) -> [];
