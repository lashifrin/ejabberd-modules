...
make_rel(Dict) ->
	...
	{ok,true} ->
		...
		case dict:find(rel_src, Dict) of
		    ...
		    %%          system ERTS
		    {ok, {release,{Name,RelVsn}, {erts, _ErtsVsn}, RelApps}} ->
...
make_config(Dict) ->
    ...
    ForeignData = lists:foldl(
		   fun({A,_V,Ebin}, Acc) when A =/= AppName ->
...
make_load_script(RelName) ->
    ...
    Keep =
	lists:filter(
	  fun({apply,{application,start_boot,[kernel,_Type]}}) -> true;
	     ({apply,{application,start_boot,[stdlib,_Type]}}) -> true;
	     ({apply,{application,start_boot,[sasl,_Type]}}) -> true;
	     ({apply,{application,start_boot,[_,_Type]}}) -> false;
	     (_) ->
		  true
	  end, Cmds),
...
merge_opts(_Opts, [{path,P}|_Opts2]) ->
...
parse_for_mod(_Fd, {ok,[{op,_,'-',{call,_,{atom,_,module},[Mod]}}],_}, _F) ->
...
parse_for_mod(Fd, {ok,_,_}, F) ->
    parse_for_mod(Fd, io:parse_erl_exprs(Fd,''), F);
parse_for_mod(_Fd, {error,_,_}=Error, F) ->
    io:format("*** Error parsing ~s: ~p~n", [F, Error]),
    error;
parse_for_mod(_Fd, {eof,_}, F) ->
...
merge_apps(RelApps, AppApps, AppInfo) ->
    ...
    Acc0 = lists:map(fun({App,_Vsn,_Incl}) ->
			     App;
			({App,_Vsn}) ->
			     App;
			(App) when atom(App) ->
...
get_newest1([], [], X1, _X2) -> X1;
get_newest1([_H1|_], [], X1, _X2) -> X1;
get_newest1([], [_H2|_], _X1, X2) -> X2.
...
member_of(AppName, [AppName|_As]) ->
    {true, AppName};
member_of(AppName, [{AppName,Vsn}|_As]) ->
    {true, {AppName, Vsn}};
